## Temas puntuales de la sección
# Temas de la sección:

```
    - Estructura y diseño de nuestra aplicación de Calendario
    - Uso de componentes de terceros
    - Modals
    - Configuración de Redux
    - CRUD local
    - Preparación de pantallas para futuras secciones
    - Uso de MomentJS
    - Manejo de fechas

Esta sección es el inicio de nuestro MERN, (Mongo Express React Node), luego crearemos nuestro backend y después regresaremos a esta app para conectarla y desplegarla en producción

```

# Apuntes

```
 Recuerda:

    1. Crear App: npx create-react-app calendar-app
    2. Eliminar de src los archivos que no se usarán (todo menos index.js)
    3. Configurar index.js
    4. Instalar React Router Dom: npm install react-router-dom
    5. Configurar App de Inicio (CalendarApp)
    6. Crear Carpetas de Componentes (components)
    7. Crear Carpetas de Router (router)

    Otros:
        - Iniciar App: npm start
        - Restaurar App: npm install
        - Instalar Sass: npm install node-sass

```

# Notas
```
    Utulitarios:

    - React Router Dom: https://v5.reactrouter.com/web/guides/quick-start
    - Bootstrap: https://getbootstrap.com/docs/4.5/getting-started/introduction/
    - font-awesome [íconos]: https://cdnjs.com/libraries/font-awesome [https://fontawesome.com/v5.15/icons?d=listing&p=7&m=free]
    - react-big-calendar [Control Calendario]: https://www.npmjs.com/package/react-big-calendar
    - moment [Manejar Fechas]: npm i moment

```

# Tarea: Agregar nuevas acciones

```


```